...
public class SketchView: UIView {
    ...
    public override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        switch currentTool! {
        case is DottedTool:
            guard let dottedTool = currentTool as? DottedTool else { return }
            pathArray.add(dottedTool)
            dottedTool.drawingPenType = drawingPenType
            dottedTool.setInitialPoint(currentPoint!)
        case is PenTool:
    ...
    public override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let penTool = currentTool as? PenTool {
            ...
            if let dottedTool = currentTool as? DottedTool {
                let renderingBox = dottedTool.createBezierRenderingBox(previousPoint2!, widhPreviousPoint: previousPoint1!, withCurrentPoint: currentPoint!)
